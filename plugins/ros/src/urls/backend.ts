...
export const LEGACY_BACKEND_URLS = {
  ...
  open_pr:
    '/api/proxy/risc-proxy/api/sops/:repoOwner/:repoName/openPullRequest/:branch', // (1)
  fetchAllRiScs:
    '/api/proxy/risc-proxy/api/risc/:repoOwner/:repoName/:version/all', // (1)
  fetchDifference:
    '/api/proxy/risc-proxy/api/risc/:repoOwner/:repoName/:id/difference', // (1)
  ...
  publishRiSc:
    '/api/proxy/risc-proxy/api/risc/:repoOwner/:repoName/publish/:id', // (1)
  fetchDefaultRiScTypeDescriptors: '/api/proxy/risc-proxy/api/initrisc',
};

/** Keep BACKEND_URLS as an alias for backward compatibility */
export const BACKEND_URLS = LEGACY_BACKEND_URLS;
 ...
 */
export function buildNativeUrls(
  baseUrl: string,
  owner: string,
  repo: string,
  version: string,

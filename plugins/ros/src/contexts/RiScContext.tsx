...
import {getTranslationKey, requiresNewApproval,} from '../utils/utilityfunctions';
import {riScRouteRef} from '../routes';
import {useLocation, useNavigate, useParams} from 'react-router';
import {dtoToRiSc, GcpCryptoKeyObject, RiScDTO, SopsConfigRequestBody,} from '../utils/DTOs';
        ...
        fetchSopsConfig(
            ...
            (_error, loginRejected) => {
                ...
                setResponse({
                    ...
                    statusMessage: loginRejected ? `${t('errorMessages.ErrorWhenFetchingSopsConfig')}. ${t('dictionary.rejectedLogin')}` : t('errorMessages.ErrorWhenFetchingSopsConfig'),
                },);
                isFetchingSopsConfigRef.current = false;
                setIsFetchingSopsConfig(isFetchingSopsConfigRef.current);
                if (!isFetchingRiScsRef.current) {
    ...
    useEffectOnce(() => {
        fetchRiScs(
            ...
            (loginRejected) => {
                setResponse({
                    status: ProcessingStatus.ErrorWhenFetchingRiScs,
                    statusMessage: loginRejected ? `${t('errorMessages.ErrorWhenFetchingRiScs')}. ${t('dictionary.rejectedLogin')}` : t('errorMessages.ErrorWhenFetchingRiScs'),
                })
                isFetchingRiScsRef.current = false;
                setIsFetchingRiScs(isFetchingRiScsRef.current);
                if (!isFetchingSopsConfigRef.current) {
    ...
    const createNewRiSc = (riSc: RiSc, generateDefault: boolean) => {
        ...
        postRiScs(
            ...
            (error, loginRejected) => {
                ...
                setResponse({
                    ...error,
                    statusMessage: loginRejected ? `${getTranslationKey('error', error.status, t)}. ${t('dictionary.rejectedLogin')}` : getTranslationKey('error', error.status, t),
                });
            },
        );
    };

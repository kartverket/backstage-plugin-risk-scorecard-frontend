...
const ConfigEncryptionDialog = ({
  ...
  useEffect(() => {
    ...
    if (publicAgeKeys.length > 0) {
      keyGroups.push({
        age: publicAgeKeys.map(key => ({ recipient: key })),
      });
    }
    setValue('sopsConfig', { shamir_threshold: 2, key_groups: keyGroups });
  }, [chosenGcpCryptoKey, publicAgeKeys, setValue]);

  // Require one key group to contain a GCP key
  register('sopsConfig.key_groups', {
    validate: keyGroups => keyGroups.some(group => group.gcp_kms),
  });

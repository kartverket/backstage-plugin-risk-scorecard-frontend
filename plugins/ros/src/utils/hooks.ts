...
import {DifferenceDTO, GithubRepoInfo, ProcessingStatus, RiSc, RiScWithMetadata, SubmitResponseObject,} from './types';
import {
    ...
    switch (currentEntity.entity.kind) {
        case 'component': {
            ...
            if (typeof associatedSystem === 'string') {
                ...
                const systemGcpProject = [system?.metadata.labels?.['gcp-access-token']]
                    .filter((value): value is string => value !== undefined)
                const associatedGcpProjectsFromOwner = await getAssociatedGcpProjectsFromOwner(componentSpec.owner)
                return Array.from(new Set([...systemGcpProject, ...associatedGcpProjectsFromOwner]))
            } else {
                return Array.from(new Set(await getAssociatedGcpProjectsFromOwner(componentSpec.owner)))
            }
        }
        case 'system': {
...
async function getAssociatedGcpProjectsFromOwner(ownerName: string): Promise<string[]> {
